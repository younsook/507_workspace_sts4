...
public class SecurityConfig {
	...
	@Autowired
	private AuthenticationConfiguration authenticationConfiguration; //시큐리티 인증 필터 등록
	
	@Autowired
	private MemberRepository memberRepository; //시큐리티 인가 필터 등록
	
	
	private final OAuth2SuccessHandler successHandler;  //final
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		// HttpBasic인증방식을 사용하지 않겠다는 명시적 설정
		// BasicAuthenticationFilter가 현재 없지만 명시적 제거
		http.httpBasic(basic->basic.disable());
		
		// 세션을유지하지않겠다고설정(SessionManagementFilter등록)
		//Url 호출 뒤 응답할 때 까지는 유지 되지만 응답 후 삭제
